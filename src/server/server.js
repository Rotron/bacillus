...
io.on('connection', function (socket) {
    ...
    socket.on('pass', function(data) {
        if (data[0] === c.adminPass) {
            ...
            socket.emit('serverMSG', 'Admin: ' + currentPlayer.name);
            ...
            for (var e = 0; e < users.length; e++) {
                if (users[e].name === data[0] && !users[e].admin && !worked) {
                    ...
                    if (reason !== '') {
                       ...
                       console.log('[ADMIN] User ' + users[e].name + ' kicked successfully by ' + currentPlayer.name + ' for some unknown reason');
                    }
                    socket.emit('serverMSG', 'User ' + users[e].name + ' was kicked by ' + currentPlayer.name + ' for ' + reason);
